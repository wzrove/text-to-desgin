...
type OutgoingRequest = { type: 'request'; id: string; method: PluginMethod; params: unknown }

type PluginRequestFrame = { type: 'request'; id: string; method: PluginMethod; params?: unknown }
...
export class PendingManager {
  ...
  request(method: PluginMethod, params: unknown, timeout = DEFAULT_TIMEOUT): Promise<unknown> {
    const id = `r${++this.seq}`
    const binary =
      params && typeof params === 'object' && 'bytes' in (params as Record<string, unknown>)
        ? ((params as Record<string, unknown>).bytes as Uint8Array)
        : null
    const payload: OutgoingRequest =
      binary != null
        ? { type: 'request', id, method, params: { ...(params as Record<string, unknown>), hasBinary: true, bytes: undefined } }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pending.set(id, { id, resolve, reject, timer, waitBinary: false, binaryCount: 0, buffers: [] })
      log(`发出请求: ${id} ${method}${binary != null ? ` +binary(${binary.byteLength}B)` : ''}`)
      this.sendFn(JSON.stringify(payload), binary != null ? Buffer.from(binary) : undefined)
    })
  }
  ...
  onText(raw: Buffer): void {
    ...
    if (msg.type === 'request') {
      if (msg.method === 'ping') {
        ...
        this.sendFn(JSON.stringify({ type: 'response', id: msg.id, ok: true, data: { pong: true } }))
      } else {
    ...
    log(`匹配响应: id=${msg.id} ok=${msg.ok}${msg.ok ? '' : ` error=${msg.error ?? ''}`}`)
  ...
  onBinary(raw: Buffer): void {
    ...
    log(`二进制帧: id=${this.binaryTarget.id} ${this.binaryTarget.buffers.length}/${this.binaryTarget.binaryCount}`)
  ...
  private settle(pending: Pending): void {
    ...
    clearTimeout(pending.timer)
    const data = pending.buffers.length === 1 ? pending.buffers[0] : pending.buffers
