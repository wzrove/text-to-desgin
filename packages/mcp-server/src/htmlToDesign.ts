...
import { isTag, type Element, type Text } from 'domhandler'
...
const COLORS: Record<string, string> = {
  ...
  blue: '#0000ff', gray: '#808080', grey: '#808080', transparent: 'none',
}
...
function color(value: string | undefined, fallback?: string): string | undefined {
...
function parseMargin(style: CssStyle): { top: number; right: number; bottom: number; left: number } {
  const s = style['margin']
  const parts = (s ?? '').split(/\s+/).filter(Boolean).map((p) => num(p) ?? 0)
...
function parsePadding(style: CssStyle): { top: number; right: number; bottom: number; left: number } {
...
function parseBorder(style: CssStyle): { width: number; color: string } {
  ...
  if (s && s !== 'none' && s !== '0') {
    ...
    if (m) return { width: parseFloat(m[1]) || 1, color: color(m[2]) ?? '#000000' }
    const m2 = s.match(/([\d.]+)(?:px)?/)
    if (m2) return { width: parseFloat(m2[1]), color: '#000000' }
  }
  if (style['border-width'] && style['border-width'] !== '0' && style['border-width'] !== 'none') {
    return {
...
function parseBox(parent: Element, root: boolean): Box {
  ...
  for (const child of parent.children) {
    if (child.type === 'text') {
      ...
      if (last && last.isText) last.text = (last.text ?? '') + t
      else box.children.push({ tag: '#text', text: t, style: inheritTextStyle(style), children: [], isText: true, x: 0, y: 0, w: 0, h: 0 })
      continue
    }
  ...
  if (root && box.children.length === 0) {
    ...
    if (t) box.children.push({ tag: '#text', text: t, style: inheritTextStyle(style), children: [], isText: true, x: 0, y: 0, w: 0, h: 0 })
  }
  return box
}
...
function inheritTextStyle(parent: CssStyle): CssStyle {
  const out: CssStyle = {}
  const keys = ['color', 'font-size', 'font-weight', 'font-family', 'line-height', 'text-align', 'letter-spacing']
  for (const k of keys) {
...
function layout(box: Box, maxWidth: number): LayoutResult {
  ...
  const innerW = Math.max(0, maxWidth - pad.left - pad.right - border.width * 2)

  if (box.tag === 'img') {
    box.w = (px(style, 'width') ?? 120)
    box.h = (px(style, 'height') ?? 120)
    return { width: box.w + margin.left + margin.right, height: box.h + margin.top + margin.bottom }
  }
  ...
  if (box.isText) {
    ...
    const textW = Math.min(innerW, Math.max(...lines.map((l) => estTextWidth(l, fs)), fs))
    box.w = textW
    ...
    if (explicitW != null) contentW = Math.max(0, explicitW - pad.left - pad.right - border.width * 2)
    ...
    if (flex && direction === 'row') {
      ...
      for (const child of box.children) {
        ...
        if (child.isText && textAlign(child.style) !== 'left') child.w = contentW - x
        x += r.width
        x += gap
      ...
      for (const child of box.children) {
        ...
        if (child.isText && textAlign(child.style) !== 'left') child.w = contentW
        y += r.height
        y += gap
...
function emit(box: Box, parts: string[], ox: number, oy: number): void {
  ...
  const bg = color(style['background'] ?? style['background-color'], box.tag === 'img' ? FALLBACK_BG : undefined)
  const border = parseBorder(style)
  ...
  if (box.tag === 'img' || visible || border.width > 0) {
    ...
    if (border.width > 0) attrs.push(`stroke="${border.color}" stroke-width="${border.width}"`)
