...
import type { FindParams } from '@jsdesign/shared'

export function findNodes(params: FindParams): Record<string, unknown> {
  const page = jsDesign.currentPage
  let nodes: SceneNode[]
  if (params.type != null) {
    nodes = page.findAllWithCriteria({ types: [params.type as NodeType] }) as SceneNode[]
  } else {
  ...
  return { nodes: nodes.slice(0, 100).map((n) => serializeNode(n, params.depth ?? 1)), total: nodes.length }
}
...
export function removeNodes(params: { ids?: string[]; matchName?: string }): Record<string, unknown> {
  let nodes: SceneNode[]
...
export function groupNodes(params: { ids: string[]; name?: string; ungroup?: boolean }): Record<string, unknown> {
  if (params.ungroup) {
