import type { PluginRequest, GetSelectionParams } from '@jsdesign/shared'
import { makeResponse } from '@jsdesign/shared'
...
function getSelection(params: GetSelectionParams = {}): Record<string, unknown> {
...
jsDesign.ui.onmessage = async (msg: PluginRequest) => {
  ...
  try {
    switch (msg.method) {
      ...
      default:
        send(id, false, undefined, `未知方法: ${(msg as { method: string }).method}`)
    }
  } catch (e) {
    console.error(e)
