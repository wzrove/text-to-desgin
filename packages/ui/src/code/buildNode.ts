...
import { makeShadowEffect, gradientPaint } from './style'

async function buildNode(spec: Spec, parent: BaseNode & ChildrenMixin): Promise<SceneNode> {
  ...
  if (spec.w != null && 'resize' in node) node.resize(spec.w, spec.h ?? node.height)
  ...
  if (spec.strokeWeight != null && 'strokeWeight' in node) node.strokeWeight = spec.strokeWeight

  if (spec.shadow && 'effects' in node) node.effects = makeShadowEffect(spec.shadow)
  ...
  if ('cornerRadius' in node) {
    ...
    if ('topLeftRadius' in node) {
      ...
      if (spec.radiusBottomLeft != null) r.bottomLeftRadius = spec.radiusBottomLeft
      if (spec.radiusBottomRight != null) r.bottomRightRadius = spec.radiusBottomRight
    }
  }

  if ((node.type === 'POLYGON' || node.type === 'STAR') && spec.pointCount != null) {
  ...
  if (node.type === 'TEXT') {
    ...
    const style = spec.fontWeight != null && spec.fontWeight >= 600 ? 'Bold' : 'Regular'
    ...
    if (spec.textAlign != null) {
      textNode.textAlignHorizontal = spec.textAlign.toUpperCase() as TextNode['textAlignHorizontal']
    }
    if (spec.lineHeight != null) textNode.lineHeight = { value: spec.lineHeight, unit: 'PIXELS' }
    if (spec.letterSpacing != null) textNode.letterSpacing = { value: spec.letterSpacing, unit: 'PIXELS' }
  }

  if (node.type === 'FRAME' && spec.layout && 'layoutMode' in node) {
    const frame = node as FrameNode
    frame.layoutMode = spec.layout.mode === 'HORIZONTAL' ? 'HORIZONTAL' : 'VERTICAL'
    frame.itemSpacing = spec.layout.itemSpacing ?? 0
    frame.paddingTop = frame.paddingRight = frame.paddingBottom = frame.paddingLeft = spec.layout.padding ?? 0
  }
