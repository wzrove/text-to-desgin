...
export function serializeNode(node: SceneNode, depth: number = MAX_SERIALIZE_DEPTH): Record<string, unknown> {
  ...
  if ('rotation' in node && node.rotation !== 0) base.rotation = Math.round(node.rotation)
  if ('opacity' in node && typeof node.opacity === 'number' && node.opacity !== 1) base.opacity = node.opacity
  ...
  if ('topLeftRadius' in node) {
    ...
    base.radiusTopLeft = typeof r.topLeftRadius === 'number' ? r.topLeftRadius : undefined
    base.radiusTopRight = typeof r.topRightRadius === 'number' ? r.topRightRadius : undefined
    base.radiusBottomLeft = typeof r.bottomLeftRadius === 'number' ? r.bottomLeftRadius : undefined
    base.radiusBottomRight = typeof r.bottomRightRadius === 'number' ? r.bottomRightRadius : undefined
  }
  if ('pointCount' in node) {
    ;(base as Record<string, unknown>).pointCount = (node as PolygonNode).pointCount
  }
  if (node.type === 'TEXT') {
    base.characters = node.characters
    if (!isMixed(node.fontSize)) base.fontSize = node.fontSize
    const f = node.fontName as FontName | undefined
    if (f && f.family) base.fontFamily = f.family
    if (f && f.style && f.style !== 'Regular') base.fontWeight = f.style
  }
  ...
  if ('children' in node && node.children.length > 0) {
    if (depth > 0) {
      base.children = node.children.map((c) => serializeNode(c as SceneNode, depth - 1))
    } else {
