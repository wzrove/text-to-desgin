import type { PluginRequest, PluginResponse } from '@jsdesign/shared'
...
export class Router {
  ...
  onWsText(conn: Conn, text: string): void {
    ...
    if (msg.type === 'request') {
      ...
      const raw = msg as PluginRequest & { params?: { hasBinary?: boolean; binaryCount?: number } }
      if (raw.params?.hasBinary) {
        conn.binaryIn = { msg, buffers: [], expected: raw.params.binaryCount ?? 1 }
        return
      }
  ...
  onWsBinary(conn: Conn, data: ArrayBuffer): void {
    ...
    if (conn.binaryIn) {
      ...
      if (conn.binaryIn.buffers.length >= conn.binaryIn.expected) {
        ...
        this.forwardRequest(conn, { ...bin.msg, params: params as PluginRequest['params'] } as PluginRequest)
      }
      return
    }
    // 未知二进制帧:丢弃
  ...
  private sendResponseOverWs(conn: Conn, id: string, ok: boolean, data?: unknown, error?: string): void {
    const bytes = extractBytes(data)
    const meta: PluginResponse =
      bytes.length > 0
        ? { type: 'response', id, ok: true, data: stripBytes(data), hasBinary: true, binaryCount: bytes.length }
        : ok
  ...
  onCodeMessage = (event: MessageEvent): void => {
    try {
      const pm = event.data?.pluginMessage as PluginRequest | PluginResponse | undefined
      if (!pm || !pm.id) return
      if (pm.type !== 'response') {
      ...
      this.log(`code 消息处理失败: ${e instanceof Error ? e.message : String(e)}`)
    }
  }
    ...
    const promise = new Promise<unknown>((resolve, reject) => {
      this.localPending.set(request.id, {
        ...
        timer: window.setTimeout(() => {
          if (this.localPending.delete(request.id)) reject(new Error('插件响应超时'))
        }, TIMEOUT),
      })
    })
